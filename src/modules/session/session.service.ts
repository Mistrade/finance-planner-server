...
export class SessionService {
  ...
  async registerUser(dto: SessionDto): Promise<TUserDocument | TProfileExceptionCodes> {
  ...
  async signInUser(
    ...
    const user: TUserDocument | null = await this.profileService.findUserByLogin(dto.login);
    ...
    try {
      await this.redisService.set(uuid, newToken, 'EX', 30 * 24 * 60 * 60);
    } catch (e: any) {
  ...
  private async getUserSession(tokenId: string | null): Promise<string | null> {
    ...
    return this.redisService.get(tokenId);
  }
